...
function initTable(){
	...
	document.getElementById("pTableCont").appendChild(table);
...
function viewElement(elem){ // Using this function costs you $11.99 (The price of a plate of spaghetti)
	...
	for (shell=0; shell<elem.shells.length; shell++){ // The shell loop.
		ringAttrs={"cx":"50%", "cy":"50%", "r":(shell+1)*5+"%", "style":"stroke: black; stroke-width:1px; fill:none;"};
		...
		for (elecN=0; elecN<elem.shells[shell]; elecN++){ // The Electron loop
			...
			elecAttrs={"cx":(50+Math.sin(angle)*(shell+1)*5)+"%", "cy":(50-Math.cos(angle)*(shell+1)*5)+"%", "r":"1%", "style":"fill: black;"};
